...
//                    }
    ...
    public void ConfirmMatch(GameSession session, MatchTypeOuterClass.MatchType matchType, boolean isAgree) {
        ...
        switch (matchType) {
            case MATCH_TYPE_DUNGEON -> {
                for (var entry : DungeonWaitingForConfirmMap.entrySet()) {
            ...
            case MATCH_TYPE_GENERAL, MATCH_TYPE_GCG -> {
                for (var entry : GeneralAndGCGWaitingForConfirmMap.entrySet()) {
                    var room = entry.getValue();
                    if (room.playersConfirmedMap.containsKey(player)) {
                        session.send(new PacketPlayerConfirmMatchRsp(matchType, room.id, isAgree));
                        ...
                        if (isAgree) {
                            ...
                            GeneralAndGCGWaitingForConfirmMap.remove(room.hostPlayer.getUid());
                            SendMatchStopMessage(room, MatchReasonOuterClass.MatchReason.MATCH_PLAYER_CANCEL);
                        }
                        entry.setValue(room);
                    }
    ...
    public synchronized void cancelMatch(GameSession session, MatchTypeOuterClass.MatchType matchType) {
        ...
        switch (matchType) {
            ...
            case MATCH_TYPE_GCG -> GCGListMap.forEach((k, v) -> {
                for (var itr = v.iterator(); itr.hasNext(); ) {
                    var room = itr.next();
                    if (room.hostPlayer == player) {
                        SendMatchStopMessage(room, MatchReasonOuterClass.MatchReason.MATCH_PLAYER_CANCEL);
                        itr.remove();
    ...
    public synchronized void onTick() {
        ...
        for (Map.Entry<Integer, Room> entry : DungeonWaitingForConfirmMap.entrySet()) {
            var room = entry.getValue();
    ...
    private static class Room {
        ...
        public Room(Player hostPlayer, int id, MatchTypeOuterClass.MatchType matchType, Player[] players) {
            this.id = id;
            this.matchType = matchType;
            this.hostPlayer = hostPlayer;
            this.hostIsMP = true;
